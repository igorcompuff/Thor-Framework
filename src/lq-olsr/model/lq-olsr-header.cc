...
namespace lqolsr {
  ...
  {
    static TypeId tid = TypeId ("ns3::lqolsr::PacketHeader")
      .SetParent<Header> ()
      .SetGroupName ("LqOlsr")
  ...
  MessageHeader::GetSerializedSize (void) const
  {
	...
	size += m_message.lqHello.GetSerializedSize ();
	break;
      case LQ_TC_MESSAGE:
      	size += m_message.lqTc.GetSerializedSize ();
      	break;
      default:
        NS_ASSERT (false);
      }
  ...
  uint32_t
  MessageHeader::LqHello::GetSerializedSize (void) const
  {
    uint32_t size = 4;
    for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
         iter != this->linkMessages.end (); iter++)
      {
        ...
        size += 4;
        size += (IPV4_ADDRESS_SIZE + 4) * lm.neighborInterfaceInformation.size ();
      }
    return size;
  }

  void
  MessageHeader::LqHello::Print (std::ostream &os) const
  {
    /// \todo
  }

  void
  MessageHeader::LqHello::Serialize (Buffer::Iterator start) const
  {
    Buffer::Iterator i = start;
    ...
    for (std::vector<LinkMessage>::const_iterator iter = this->linkMessages.begin ();
         iter != this->linkMessages.end (); iter++)
      {
        ...
        i.WriteHtonU16 (4 + (lm.neighborInterfaceInformation.size () * (IPV4_ADDRESS_SIZE + 4)));

        for (std::vector<NeighborInterfaceInfo>::const_iterator neigh_info = lm.neighborInterfaceInformation.begin ();
             neigh_info != lm.neighborInterfaceInformation.end (); neigh_info++)
          {
            i.WriteHtonU32 (neigh_info->neighborInterfaceAddress.Get ());
            i.WriteHtonU32 (neigh_info->metricInfo);
          }
      }
  }

  uint32_t
  MessageHeader::LqHello::Deserialize (Buffer::Iterator start, uint32_t messageSize)
  {
    Buffer::Iterator i = start;
    ...
    while (helloSizeLeft)
      {
        ...
        NS_ASSERT ((lmSize - 4) % (IPV4_ADDRESS_SIZE + 4) == 0);
        NeighborInterfaceInfo neigh_info;
        for (int n = (lmSize - 4) / (IPV4_ADDRESS_SIZE + 4); n; --n)
          {
            neigh_info.neighborInterfaceAddress = Ipv4Address (i.ReadNtohU32 ());
            neigh_info.metricInfo = i.ReadNtohU32 ();
            lm.neighborInterfaceInformation.push_back (neigh_info);
          }
  ...
  void
  MessageHeader::LqTc::Print (std::ostream &os) const
  {
    /// \todo
  }

  void
  MessageHeader::LqTc::Serialize (Buffer::Iterator start) const
  {
    Buffer::Iterator i = start;
  ...
  uint32_t
  MessageHeader::LqTc::Deserialize (Buffer::Iterator start, uint32_t messageSize)
  {
    Buffer::Iterator i = start;
